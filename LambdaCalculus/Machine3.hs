...
module LambdaCalculus.Machine3 (
    SECD(SECD), SECD1
  ) where
...
instance  HasState (SECD s e c d) where
  type State (SECD s e c d) = SECDState s e c d
  state g (SECD s e c d tr) = fmap (\(SECDState x1 x2 x3 x4) -> SECD x1 x2 x3 x4 tr) (g (SECDState s e c d))
